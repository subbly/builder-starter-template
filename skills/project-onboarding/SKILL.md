...
The flow has three gates: intake confirmation (Step 1), design-system pick (Step 3), hero pick (Step 5). Each gate is an `ask_question` call the user must answer before the flow continues. A gate never passes on inference: even when the prompt implies an answer, ask. Only two exceptions close a gate without asking: copy mode (gates 2 and 3, see below) and a provided inspiration image (gate 3, see Step 4).
...
Issue ONE `ask_question` call with exactly these three questions, in order:

1. Shop name.
2. Business model.
3. Business category.

Bake the best guess into each question text (e.g. `Shop name (current: "Wildmoor Botanicals")`), single option `Yes, that's right`, user corrects via free text. Best guess: initial prompt first, project-reminder Shop Information as fallback; a known answer changes the guess, never whether you ask. 

Gate 1: all three confirmed or corrected before Step 2.
...
- If the user provided brand colors or a palette (including one extracted from an image), all four stay on it: one exact (derive only unspecified token slots), three with slight shade variation at most. The color rules above do not apply; differentiate on the non-color dimensions instead.
